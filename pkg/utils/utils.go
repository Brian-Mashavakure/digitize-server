...
func MultipleMarkdownsToPDF(markdowns []string, fontPath string) ([]byte, error) {
	if len(markdowns) == 0 {
		return nil, fmt.Errorf("no markdowns provided")
	}
	...
	for markdownIndex, markdown := range markdowns {
		if markdownIndex > 0 {
			pdf.AddPage()
		} else {
			pdf.AddPage()
		...
		err := processMarkdownToPDF(&pdf, markdown)
		if err != nil {
			return nil, fmt.Errorf("failed to process markdown %d: %w", markdownIndex+1, err)
		}
	}
...
func processMarkdownToPDF(pdf *gopdf.GoPdf, markdown string) error {
	lines := strings.Split(markdown, "\n")
	yPos := 40.0
	marginX := 30.0
	lineHeight := 20.0
	...
	for _, line := range lines {
		...
		if linkRegex.MatchString(line) {
			...
			for _, match := range matches {
				...
				linkWidth, err := pdf.MeasureTextWidth(linkText)
				if err != nil {
					return fmt.Errorf("failed to measure link text width: %w", err)
				}
		...
		if textWidth > maxWidth {
			...
			for _, word := range words {
				...
				testWidth, err := pdf.MeasureTextWidth(testLine)
				if err != nil {
					return fmt.Errorf("failed to measure test line width: %w", err)
				}
				if testWidth > maxWidth {
					if currentLine != "" {
						pdf.SetXY(marginX, yPos)
